...
- Aligning rule provenance with the standing citation rule: the current catalog cites `HS-` source-provenance slugs such as `HS-PURPOSE-10` rather than sections of an authored criteria document, so project rule 9 (every rule cites a criteria-document section) is not satisfied as written; either the rules gain criteria-section citations or the rule is restated to match how provenance now works.
