...
**Read this file and `docs/rules/README.md`. Nothing else.**
...
independent attributes in `docs/design/02-evaluation-model.md` §1.
...
`docs/rules/python.md`. `Impact` stays empty — it is derived in Phase 5.
...
and contradicted `docs/design/06-cross-language.md`. Golden fact fixtures are stored under
...
- [ ] Every rule in `docs/rules/python.md` has matching catalog metadata; a test
...
2. **Claim assignments were invented** rather than taken from
   `docs/rules/python.md`. Check every row.
