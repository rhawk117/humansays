...
``py_ast`` owns the AST vocabulary and fact extraction; this module owns the
judgement calls. The split is the advice PY017 gives — the combined
implementation crossed the module line threshold and the scanner flagged it
while running against itself.
"""
...
class Analyzer:
    def __init__(
        ...
        vocabulary: MutationVocabulary = MutationVocabulary(),
    ) -> None:
        self.module = module
    ...
    def run(self) -> list[Finding]:
        ...
        self.findings.extend(future_annotation_findings(self.module))
    ...
    def _static_method(self, node: ast.AST, qualified_name: str) -> None:
        """PY015: a staticmethod is a module function wearing a class as a namespace."""
        if STATIC_DECORATOR not in decorator_names(node):
            return
        observation = Observation(
            "Method is declared @staticmethod, so it can reach neither instance nor class state.",
            (f"line {node.lineno}: @staticmethod {node.name}",),
        )
        self._record(SignalName.PY015, location_of(qualified_name, node), observation)

    def _lambda_signals(self) -> None:
        """PY016: lambdas are anonymous, unimportable, and awkward to test."""
        for node in ast.walk(self.module.tree):
            ...
            self._record(SignalName.PY016, location_of(scope.symbol, node), observation)

    def _base_classes(self, node: ast.ClassDef) -> None:
        """PY018: multiple parents make the method resolution order the real design."""
        ...
        self._record(SignalName.PY018, location_of(node.name, node), observation)
    ...
    def _validated_bundle(self, facts: FunctionFacts) -> None:
        ...
        if not names:
            return
        evidence = tuple(
            f"{parameter}: {sorted(validated[parameter])[0]}" for parameter in names
        )
        self._record(
            SignalName.PY014,
            facts.location,
    ...
    def _incident_signals(self, facts: FunctionFacts) -> None:
        messages = {
            SignalName.PY005: "Broad exception handling may collapse unrelated failures.",
            SignalName.PY021: "Import is deferred into the function body.",
        }
    ...
    def _state_signals(self, facts: FunctionFacts) -> None:
        ...
        if len(mutations) >= MUTATION_OWNER_MINIMUM:
            evidence = tuple(
                f"{owner}: {sorted(details)[0]}"
                for owner, details in sorted(mutations.items())
            )
            self._record(
                SignalName.PY006,
                facts.location,
                Observation(
                    f"Function appears to mutate {len(mutations)} independent state owners.",
                    evidence,
                ),
            )
        ...
        if len(boundaries) >= BOUNDARY_MINIMUM:
            ...
            self._record(
                ...
                facts.location,
                Observation(
                    f"Function uses {len(boundaries)} standard-library boundary categories.",
                    evidence,
                ),
            )
        ...
        for statement in body:
            ...
            for name, value in assigned_names(statement):
                ...
                self._record(
                    ...
                    Observation(
                        f"Mutable {scope}-scope state `{name}` is shared beyond one instance or operation.",
    ...
    def _class_state_surface(
        ...
        if clusters:
            ...
            self._record(
                ...
                location,
                Observation(
                    f"Large class contains {len(clusters)} repeated attribute-prefix clusters.",
                    evidence,
                ),
            )
    ...
    def _class_cohesion(
        ...
        self._record(
            ...
            Observation(
                f"Class methods form {len(components)} disconnected field-access clusters.",
                tuple(evidence),
            ),
        )

    def _documentation_signals(self) -> None:
        scopes_by_symbol = {scope.symbol: scope for scope in self.index.scopes}
        for symbol, lines in self._comment_lines().items():
            scope = scopes_by_symbol[symbol]
            evidence = tuple(f"comment line {number}" for number in lines[:EVIDENCE_LIMIT])
            if len(lines) > EVIDENCE_LIMIT:
                evidence += (f"{len(lines) - EVIDENCE_LIMIT} additional comments",)
            self._record(
                SignalName.PY010,
                Location(symbol, scope.line, scope.end_line),
                Observation(f"Scope contains {len(lines)} comments.", evidence),
            )
        for scope in self.index.scopes:
            if ast.get_docstring(scope.node, clean=False) is None:
                continue
            first = scope.node.body[0]
            self._record(
                SignalName.PY011,
                Location(scope.symbol, scope.line, scope.end_line),
                Observation(
                    "Scope contains a docstring.",
                    (f"docstring starts at line {first.lineno}",),
                ),
            )

    def _comment_lines(self) -> dict[str, list[int]]:
        comments: dict[str, list[int]] = {}
        readline = io.StringIO(self.module.source).readline
        try:
            for token in tokenize.generate_tokens(readline):
                ...
                if line == 1 and token.string.startswith("#!"):
                    continue
                symbol = self.index.scope_for_line(line).symbol
                comments.setdefault(symbol, []).append(line)
