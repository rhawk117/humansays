...
from .const import BOOL_NAMES, BOUNDARY_MODULES, UNPARSE_LIMIT
from .models import Location, ParsedModule
...
def contains_raise(nodes: Iterable[ast.AST]) -> bool:
    return any(
        isinstance(child, ast.Raise) for node in nodes for child in ast.walk(node)
    )
...
def classify_boundary(name: str) -> str | None:
    for boundary, modules in BOUNDARY_MODULES.items():
        matches = (
            name == module or name.startswith(f"{module}.") for module in modules
        )
        if any(matches):
            return boundary
    return None
